...
UserSchema.statics.userhaspermission = function (userId, callback) {
  ...
  .exec(function (error, ActualUser) {
    if (error) {
      ...
      if (ActualUser === null) {
        ...
        if( ActualUser.perfil == "Administrador" || ActualUser.perfil == "Gerente" ){
          return callback(null, 1,ActualUser);
        }else{
          return callback(null, 2,ActualUser);
        }        
      }
     }
  });
